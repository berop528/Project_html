...
function numberToThaiWords(number) {
    ...
    if (number === 0) {
        return thaiNumbers[0]; // ถ้าเป็น 0
    }
    ...
    while (number > 0) {
        ...
        if (digit > 0) {
            if (digit === 1 && position === 1) {
                result = thaiPositions[position] + result;
            } else {
                result = thaiNumbers[digit] + thaiPositions[position] + result;
            }
        }
...
const totalValue = 1234567.89; // ตัวอย่างค่า total
const thaiWords = numberToThaiWords(totalValue);

console.log(thaiWords); // แสดงผลลัพธ์ในรูปแบบข้อความภาษาไทย
